...
const Player = (sign) => {
    ...
    const setSign = (sign, active) => {
        _sign = sign;
        const player = document.querySelector(`.player-btn.${sign.toLowerCase()}`);
        if(active){
            player.classList.add('chosen');
            player.classList.remove('unchosen');
        }
        else{
            player.classList.remove('chosen');
            player.classList.add('unchosen');
        }
    }
    return {
        getSign,
...
const minimaxAiLogic = ((percentage) => {
    ...
    const minimax = (newBoard, player) => {
        ...
        for (let i = 0; i < availableFields.length; i++) {
            ...
            newBoard.setFieldForAiLogic(availableFields[i], player);
            ...
            newBoard.setFieldForAiLogic(availableFields[i], undefined);
...
const displayController = (() => {
    ...
    const _switchAI = () => {
        ...
        if (value == 'easy') {
            ...
            minimaxAiLogic.setAiPercentage(65);
        }
        else if (value == 'hard') {
